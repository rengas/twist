...
// TaskEvent represents a workflow event recorded during kanban pipeline execution.
...
// ChatTimelineEntry is a union type for the chat timeline, combining events and messages.
type ChatTimelineEntry struct {
	Type      string       `json:"type"`              // "event" | "message"
...
func handlePrompt(task Task, workDir string, repo Repository, designMu *sync.Mutex, log LogFunc) error {
	...
	// Record prompt submission event.
	_ = repo.InsertTaskEvent(task.ID, "prompt_submitted", "user", "Prompt submitted", task.Prompt)
...
Return ONLY the markdown spec, no other commentary.`, task.Prompt, contextBlock)
	...
	if err != nil {
		_ = repo.InsertTaskEvent(task.ID, "failed", "system", "Spec generation failed", err.Error())
		return fmt.Errorf("spec generation failed for task #%d: %v", task.ID, err)
	}

	trimmedSpec := strings.TrimSpace(spec)

	// Record spec generated event.
	_ = repo.InsertTaskEvent(task.ID, "spec_generated", "agent", "Spec generated by Claude", trimmedSpec)
...
func handleCode(task Task, workDir string, repo Repository, designMu *sync.Mutex, log LogFunc) error {
	...
	// Record code started event.
	_ = repo.InsertTaskEvent(task.ID, "code_started", "system", fmt.Sprintf("Code phase started on branch `%s`", branch), branch)
...
Do not ask for permission — implement, test, and commit.`, task.Spec, branch)
	...
	if err := runClaudeStream(task.ID, prompt, wtPath, task.SessionID, log); err != nil {
		...
		_ = repo.InsertTaskEvent(task.ID, "failed", "system", "Code phase failed", err.Error())
		_ = repo.UpdateTaskStatus(task.ID, "failed", false)
		return nil
	}

	// Record code completed event.
	_ = repo.InsertTaskEvent(task.ID, "code_completed", "agent", "Code implementation completed", "")
	...
	if out, err := gitCmd(wtPath, "push", "-u", "origin", branch); err != nil {
		...
		_ = repo.InsertTaskEvent(task.ID, "failed", "system", "Git push failed", fmt.Sprintf("%v\n%s", err, out))
		_ = repo.UpdateTaskStatus(task.ID, "failed", false)
		return nil
	}
	...
	if err != nil {
		...
		_ = repo.InsertTaskEvent(task.ID, "failed", "system", "PR creation failed", fmt.Sprintf("%v\n%s", err, prOut))
		_ = repo.UpdateTaskStatus(task.ID, "failed", false)
		return nil
	}
	...
	_ = repo.InsertTaskEvent(task.ID, "pr_created", "system", fmt.Sprintf("PR created: %s", prURL), prURL)
